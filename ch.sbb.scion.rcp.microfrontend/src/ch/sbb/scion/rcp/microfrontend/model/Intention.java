...
public class Intention {
   ...
   *
   * The intention must exactly match the qualifier of the capability, if any. The
   * intention qualifier allows using wildcards (such as `*` or `?`) to match
   * multiple capabilities simultaneously.
   *
   * In the intention, the following wildcards are supported: - **Asterisk
   * wildcard character (`*`):**\ Matches capabilities with such a qualifier
   * property no matter of its value (except `null` or `undefined`). Use it like
   * this: `{property: '*'}`. - **Optional wildcard character (?):**\ Matches
   * capabilities regardless of having or not having such a property. Use it like
