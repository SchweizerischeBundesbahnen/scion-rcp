...
public class MessageInterceptorInstaller {
  ...
  private <T> void registerInterceptor(JavaCallback interceptorCallback,
      MessageInterceptorDescriptor<T> interceptorDescriptor, Browser hostBrowser) {
  ...
  private <T> JavaCallback createJavaInterceptorCallback(MessageInterceptorDescriptor<T> interceptorDescriptor,
    ...
    return new JavaCallback(hostBrowser, args -> {
      TopicMessage<T> message = GsonFactory.create()
          .fromJson((String) args[0], new ParameterizedType(TopicMessage.class, interceptorDescriptor.payloadClazz));
