...
public class IntentInterceptorInstaller {
   ...
   */
  private <T> void registerInterceptor(JavaCallback interceptorCallback,
      IntentInterceptorDescriptor<T> interceptorDescriptor, Browser hostBrowser) {
    new JavaScriptExecutor(hostBrowser, Resources.readString("js/registerInterceptor.js"))
   ...
   */
  private <T> JavaCallback createJavaInterceptorCallback(IntentInterceptorDescriptor<T> interceptorDescriptor,
      Browser hostBrowser) {
    return new JavaCallback(hostBrowser, args -> {
      IntentMessage<T> intent = GsonFactory.create()
          .fromJson((String) args[0], new ParameterizedType(IntentMessage.class, interceptorDescriptor.payloadClazz));
